...
learned_word=list(range(0,word_cnt//10)) # range(0,파트 개수) -> list에는 각 part에서의 배운 단어 개수가 들어간다.
# 단어장 화면에서 사이드바 여는 함수
...
def create_dic_part(frame,current_window,part_index):
    ...
    progress_label = Label(white_box, text=f"학습률: {learned_word[part_index]} / 120 [ {learned_word[part_index]//120*100}% ]",background="white",font="Helvetica") #
    progress_label.place(relx=0.55,rely=0.73)

    progress_bar = Progressbar(white_box, orient="horizontal", mode="determinate",length=300)
    progress_bar['value']= learned_word[part_index]//120*100 # 학습률 넣기
